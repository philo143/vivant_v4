...
page.onPageCreated = function(newPage) {
	...
	var last = fs.exists(script_loc+'RTD_LMP_'+participant+'_LastData.txt') ? fs.read(script_loc+'RTD_LMP_'+participant+'_LastData.txt') : fs.write(script_loc+'RTD_LMP_'+participant+'_LastData.txt',new Date(),'w');
	if(args[3] == 'last'){
		args[3] = last;
  		...
  		newPage.includeJs("http://ajax.googleapis.com/ajax/libs/jquery/1.6.1/jquery.min.js",function(){
  			waitFor(function(){
		  		...
		  		waitFor(function(){
		  			...
		  			newPage.evaluate(function(){
			  			document.getElementById('DropdownMenu.xMoarDa.moarRtdOutputDisplays.xRtdLMPs').click(); // <-- RTD LMPs LINK
			  		})
			  		// newPage.render('regional_summary.png');
		  			waitFor(function(){
			  			return newPage.evaluate(function(){
						...
						var retry = 1;
			  			var data_interval = function(){			  				
							...
							var output =  ((''+month).length<2 ? '0' : '') + month + '/' +
		  					...
		  					waitFor(function(){
		  						return newPage.evaluate(function(output,last_d){ //|| ($('.rowSelectionMode tbody tr:first-child td:first-child span').text() == output -> incase opacity and eventingIcon is not reliable
								...
								if(result != false){
									waitForData(function(){										
										...
										if(xls_data && dl_status == 'success'){// SAVE AND CONTINUE
											retry = 1;
				  							fs.write(script_loc+'mms_mod/RTD_LMP_'+participant+'_'+last_output+'.xls', xls_data,'w');
				  							if(last < last_d){
							  					fs.write(script_loc+'RTD_LMP_'+participant+'_LastData.txt',last_d,'w');
							  				}
											...
											if(retry != 5){
												...
												retry = 1;		  							
								  				if(latest_d > last_d){
						  							fs.write(script_loc+'mms_mod/no_data/RTD_LMP_'+participant+'_'+last_output+'_NO_DATA.json','NO DATA','w');						  							
										  			last_d.setMinutes(last_d.getMinutes() + 5);	
										  		}	
