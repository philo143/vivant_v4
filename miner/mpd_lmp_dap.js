// MARKET PROJECTIONS DISPLAYS -> LMPs (DAP)
...
page.onPageCreated = function(newPage) {
	...
	var last = fs.exists(script_loc+'MPD_LMPS_DAP_'+participant+'_LastData.txt') ? fs.read(script_loc+'MPD_LMPS_DAP_'+participant+'_LastData.txt') : fs.write(script_loc+'MPD_LMPS_DAP_'+participant+'_LastData.txt',new Date(),'w');
	if(args[3] == 'last'){
		args[3] = last;
  		...
  		newPage.includeJs("http://ajax.googleapis.com/ajax/libs/jquery/1.6.1/jquery.min.js",function(){
  			waitFor(function(){
		  		...
		  		waitFor(function(){
		  			...
		  			newPage.evaluate(function(){
			  			document.getElementById('DropdownMenu.xMoarDa.moarOutputDisplays.xMoarLMPs').click(); // <-- Market Projections Displays -> LMPs
			  		})
		  			waitFor(function(){
			  			return newPage.evaluate(function(){
			  				if($('#progressBar').css('opacity') == '0' && $('#NAV_CELL_TITLE tbody tr td:nth-child(5)').text() == 'LMPs'){
			  					return true;
			  				}
			  				return false;
			  			})
				  			...
				  			var data_interval = function(){			  				
								...
								var output =  ((''+month).length<2 ? '0' : '') + month + '/' +
		  						...
		  						waitForData(function(){				
			  						return newPage.evaluate(function(output){
						  				if($('#progressBar').css('opacity') == '0' && $("#eventingIcon").hasClass("WFConnectedIcon")){
						  					return true;
						  				}
						  				return false;
						  			},output)
									...
									// LAST DATE WITH DATA FOR DAY AHEAD = -2 days eg. (Today = August 28, 2017) Last date with data will be August 26, 2017. 
									latest_d = new Date();
									latest_d.setDate(latest_d.getDate() - 2);
									if(result != false){
										waitForData(function(){
											if(xls_data != null){
												return true;
											...
											if(xls_data && dl_status == 'success' || xls_data.search('No Data') == false){// SAVE AND CONTINUE
												retry = 1;
					  							fs.write(script_loc+'mms_mpd/MPD_LMPS_DAP_'+participant+'_'+last_output+'.xls', xls_data,'w');
					  							if(last < last_d){
								  					fs.write(script_loc+'MPD_LMPS_DAP_'+participant+'_LastData.txt',last_d,'w');
								  				}
								  				last_d.setHours(last_d.getHours() + 1);	
												...
												if(retry != 5){
													...
													retry = 1;		  							
									  				if(latest_d > last_d){
							  							fs.write(script_loc+'mms_mpd/no_data/MPD_LMPS_DAP_'+participant+'_'+last_output+'_NO_DATA.json','NO DATA','w');						  							
											  			last_d.setHours(last_d.getHours() + 1);
											  			setTimeout(data_interval,5000);
											  		}else{
