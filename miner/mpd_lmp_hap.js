// MARKET PROJECTIONS DISPLAYS -> LMPs (HAP)
...
page.onPageCreated = function(newPage) {
	...
	var last = fs.exists(script_loc+'MPD_LMPS_HAP_'+participant+'_LastData.txt') ? fs.read(script_loc+'MPD_LMPS_HAP_'+participant+'_LastData.txt') : fs.write(script_loc+'MPD_LMPS_HAP_'+participant+'_LastData.txt',new Date(),'w');
	if(args[3] == 'last'){
		args[3] = last;
		  		...
		  		waitFor(function(){
		  			...
		  			waitFor(function(){
						...
						// }
							...
							last = new Date(last);
							last.setMinutes(last.getMinutes() - 5); // REMOVE WHEN LIVE (AS OF 06/04/2018 HAP IS DELAY BY 5 mins)
							var retry = 1;
									...
									if(result != false){
										waitForData(function(){										
											...
											if(xls_data && dl_status == 'success'){// SAVE AND CONTINUE
												retry = 1;
					  							fs.write(script_loc+'mms_mpd/MPD_LMPS_HAP_'+participant+'_'+last_output+'.xls', xls_data,'w');
					  							if(last < last_d){
								  					fs.write(script_loc+'MPD_LMPS_HAP_'+participant+'_LastData.txt',last_d,'w');
								  				}
												...
												if(retry != 5){
													...
													retry = 1;		  							
									  				if(latest_d > last_d){
							  							fs.write(script_loc+'mms_mpd/no_data/MPD_LMPS_HAP_'+participant+'_'+last_output+'_NO_DATA.json','NO DATA','w');						  							
											  			last_d.setMinutes(last_d.getMinutes() + 5);	
											  		}	
