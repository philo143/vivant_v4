// MARKET PROJECTIONS DISPLAYS -> LMPs (WAP)
...
page.onPageCreated = function(newPage) {
	...
	var last = fs.exists(script_loc+'MPD_LMPS_WAP_'+participant+'_LastData.txt') ? fs.read(script_loc+'MPD_LMPS_WAP_'+participant+'_LastData.txt') : fs.write(script_loc+'MPD_LMPS_WAP_'+participant+'_LastData.txt',new Date(),'w');
	if(args[3] == 'last'){
		args[3] = last;
				  			...
				  			var data_interval = function(){			  				
								...
								var output =  ((''+month).length<2 ? '0' : '') + month + '/' +
		  						...
		  						waitForData(function(){				
									...
									// LAST DATE WITH DATA FOR WEEK AHEAD = -8 days eg. (Today = August 28, 2017) Last date with data will be August 20, 2017. 
									latest_d = new Date();
									latest_d.setDate(latest_d.getDate() - 8);
									if(result != false){
										waitForData(function(){
											if(xls_data != null){
												return true;
											...
											if(xls_data && dl_status == 'success'){// SAVE AND CONTINUE
												retry = 1;
					  							fs.write(script_loc+'mms_mpd/MPD_LMPS_WAP_'+participant+'_'+last_output+'.xls', xls_data,'w');
					  							if(last < last_d){
								  					fs.write(script_loc+'MPD_LMPS_WAP_'+participant+'_LastData.txt',last_d,'w');
								  				}
								  				last_d.setDate(last_d.getDate() + 1);
												...
												if(retry != 5){
													...
													retry = 1;		  							
									  				if(latest_d > last_d){
							  							fs.write(script_loc+'mms_mpd/no_data/MPD_LMPS_WAP_'+participant+'_'+last_output+'_NO_DATA.json','NO DATA','w');						  							
											  			last_d.setDate(last_d.getDate() + 1);
											  		}	
