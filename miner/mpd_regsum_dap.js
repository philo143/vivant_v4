// MARKET PROJECTIONS DISPLAYS -> Regional Summary (DAP)
...
page.onPageCreated = function(newPage) {
	...
	var last = fs.exists(script_loc+'MPD_REGSUM_DAP_'+participant+'_LastData.txt') ? fs.read(script_loc+'MPD_REGSUM_DAP_'+participant+'_LastData.txt') : fs.write(script_loc+'MPD_REGSUM_DAP_'+participant+'_LastData.txt',new Date(),'w');
	if(args[3] == 'last'){
		args[3] = last;
						...
						var output =  ((''+month).length<2 ? '0' : '') + month + '/' +
	  					...
	  					newPage.evaluate(function(output){
				  			...
				  			document.getElementById('ApplyButton').click();
				  				console.log('CLICK APPLY BUTTON'); // CLICK APPLY ApplyButton
	  					},output)
	  					var reclick_time = 0;
  						waitForData(function(){				
	  						...
	  						var xls_data = null;
	  						var dl_status = null;
	  						// newPage.render('mpd-REGSUM_dap.png');
							...
							if(result != false){
								waitForData(function(){
									...
									if(xls_data && dl_status == 'success' || xls_data.search('No Data')){// SAVE AND CONTINUE
										retry = 1;
			  							fs.write(script_loc+'mms_mpd/MPD_REGSUM_DAP_'+participant+'_'+last_output+'.xls', xls_data,'w');
			  							if(last < last_d){
						  					fs.write(script_loc+'MPD_REGSUM_DAP_'+participant+'_LastData.txt',last_d,'w');
						  				}
										...
										if(retry != 5){
											...
											retry = 1;		  							
							  				if(latest_d > last_d){
					  							fs.write(script_loc+'mms_mpd/no_data/MPD_REGSUM_DAP_'+participant+'_'+last_output+'_NO_DATA.json','NO DATA','w');						  							
									  			last_d.setHours(last_d.getHours() + 1);
									  			setTimeout(data_interval,5000);
									  		}else{
