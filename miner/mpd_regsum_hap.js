// MARKET PROJECTIONS DISPLAYS -> Regional Summary (HAP)
...
page.onPageCreated = function(newPage) {
	...
	var last = fs.exists(script_loc+'MPD_REGSUM_HAP_'+participant+'_LastData.txt') ? fs.read(script_loc+'MPD_REGSUM_HAP_'+participant+'_LastData.txt') : fs.write(script_loc+'MPD_REGSUM_HAP_'+participant+'_LastData.txt',new Date(),'w');
	if(args[3] == 'last'){
		args[3] = last;
							...
							if(result != false){
								waitForData(function(){										
									...
									if(xls_data && dl_status == 'success'){// SAVE AND CONTINUE
										retry = 1;
			  							fs.write(script_loc+'mms_mpd/MPD_REGSUM_HAP_'+participant+'_'+last_output+'.xls', xls_data,'w');
			  							if(last < last_d){
						  					fs.write(script_loc+'MPD_REGSUM_HAP_'+participant+'_LastData.txt',last_d,'w');
						  				}
										...
										if(retry != 5){
											...
											retry = 1;		  							
							  				if(latest_d > last_d){
					  							fs.write(script_loc+'mms_mpd/no_data/MPD_REGSUM_HAP_'+participant+'_'+last_output+'_NO_DATA.json','NO DATA','w');						  							
									  			last_d.setMinutes(last_d.getMinutes() + 5);	
									  		}	
