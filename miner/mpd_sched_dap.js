// MARKET PROJECTIONS DISPLAYS -> SCHEDULES (DAP)
...
page.onPageCreated = function(newPage) {
	...
	var last = fs.exists(script_loc+'MPD_SCHED_DAP_'+participant+'_LastData.txt') ? fs.read(script_loc+'MPD_SCHED_DAP_'+participant+'_LastData.txt') : fs.write(script_loc+'MPD_SCHED_DAP_'+participant+'_LastData.txt',new Date(),'w');
	if(args[3] == 'last'){
		args[3] = last;
									...
									if(result != false){
										waitForData(function(){
											...
											if(xls_data && dl_status == 'success' || xls_data.search('No Data')){// SAVE AND CONTINUE
												retry = 1;
					  							fs.write(script_loc+'mms_mpd/MPD_SCHED_DAP_'+participant+'_'+last_output+'.xls', xls_data,'w');
					  							if(last < last_d){
								  					fs.write(script_loc+'MPD_SCHED_DAP_'+participant+'_LastData.txt',last_d,'w');
								  				}
												...
												if(retry != 5){
													...
													retry = 1;		  							
									  				if(latest_d > last_d){
							  							fs.write(script_loc+'mms_mpd/no_data/MPD_SCHED_DAP_'+participant+'_'+last_output+'_NO_DATA.json','NO DATA','w');						  							
											  			last_d.setHours(last_d.getHours() + 1);
											  			setTimeout(data_interval,5000);
											  		}else{
