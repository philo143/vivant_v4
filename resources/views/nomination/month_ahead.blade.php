...
<div class="container-fluid">
	...
	<div class="row">
	        ...
	        <div class="col-md-10">
	            {!! Breadcrumbs::render('month_ahead_nomination') !!}
				@include ('user.message')
				<h4>Month Ahead Nomination</h4>
	            	             
	            <div class="well bs-component col-md-12">
		            {{ Form::open(['route' => 'nomination.month_ahead.data', 'id'=>'form_retrieve', 'class'=>'form-horizontal']) }}
					...
					<div class="form-group"> 
                        {{ Form::label('delivery_date', 'Billing Period:', ['class'=>'col-lg-2 control-label']) }}
                        <div class="col-lg-2 input-group" style="padding-left:15px; float:left;"> 
                            {{ Form::select('billing_month', $months, $billing_month, ['class'=>'form-control input-sm' , 'id' => 'billing_month']) }}
                        </div>

                        <div class="col-lg-1 input-group" style="padding-left:15px;"> 

                            {{ Form::select('billing_year', $years, $billing_year, ['class'=>'form-control input-sm', 'id' => 'billing_year']) }}
	            ...
	            <div class="well col-md-12">
					...
					<div id="collapseOne" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne" style="height: 0px;">
					      <div class="panel-body">
					        {{ Form::open(array('route'=>'nomination.man.upload','files'=>true,'method'=>'POST'),['class'=>'form-horizontal']) }}
					        <input type="hidden" name="upload_month" id="upload_month" value="">
					        <input type="hidden" name="upload_year" id="upload_year" value="">
						...
						{{ Form::open(['id'=>'form_populate', 'class'=>'form-horizontal']) }}
					      <div class="panel-body">
							...
							<div class="form-group">
		                        ...
		                        <div class="col-lg-9 row" id="checkbox_container" style="margin-top:10px;">
		                        	@if ( $is_with_uploading == 1 )

		                        		@foreach ($date_list as $date)
										    <div class="col-sm-2"><label><input type="checkbox" class="week_date" name="week_date"  value="{{$loop->index}}" checked=true>&nbsp;{{$date}}&nbsp;&nbsp;</label>&nbsp; </div>
										@endforeach
		                        	@endif
		                        </div>
		                    </div>
	            ...
	            <div class="well col-md-12">
	            	{{ Form::open(['route' => 'nomination.month_ahead.store', 'class'=>'form-horizontal']) }}
...
// $(document).ready(function(){
	$.extend({
		retrieve : function(){
            $.ajax({
                url : "/nomination/month_ahead/data",                 
                ...
                success : function(data){
                	...
                	$.each(data.date, function(k, cur_date){
                		...
                		active = (i === 0) ? 'active' : '';
                		date_checkbox+='<div class="col-sm-2"><label><input type="checkbox" class="week_date" name="week_date"  value="'+i+'" checked=true>&nbsp;'+cur_date+'&nbsp;&nbsp;</label>&nbsp;</div>'; 
        ...
        total_nominations : function(){
        	var max = parseInt($('#total_days').val(),10);
        	for(var w=0;w<=max;w++){
        		var sum = 0;
        ...
        , populateDates : function(){
			...
			var tmp = billing_year +'-'+ billing_month +'-01';

			var start_date = moment(tmp).subtract(1,'months').format('M/26/YYYY');
			var end_date = moment(tmp).format('M/25/YYYY');
    ...
    $('#month').val($('#billing_month').val());
    $('#year').val($('#billing_year').val());
    $('#upload_month').val($('#billing_month').val());
    $('#upload_year').val($('#billing_year').val());
    
    $('#billing_year,#billing_month').unbind().bind('change',function(){
    	$.populateDates();
