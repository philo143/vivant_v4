	    ...
	    <div class="row">
	        ...
	        <div class="col-md-10">
	            ...
	            <div class="well col-md-12">
			      	<h5>Upload Energy Offer Template &nbsp;&nbsp;
					...
					<div id="collapseOne" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
					      <div class="panel-body">
					        ...
					        {{ Form::hidden('template_type','energy')}}
			                    ...
			                    <tbody>																											
									@for($i=1; $i<=24; $i++)								
										<tr id="tr{{ $i }}">
				                            ...
				                            <td>
				                                <input name="web-ramp_rate-{{ $i }}" type="text" class="form-control ramp_rate input-sm" value="{{ $form_data['intervals'][$i]['ramp_rate']  ? $form_data['intervals'][$i]['ramp_rate'] : ''}}"/>
				                            </td>
				                            {{-- <td>
   ...
   $(document).ready(function(){
	    $.extend({
			...
			determineDisabled : function(){
                ...
                var svr_datetime = moment().hours(svr_time_full).format('YYYY-MM-DD HH');
                var svr_datetime = moment(svr_datetime).add(3, 'hours').format('YYYYMMDDHH');
                var sel_date = $('#delivery_date').val();
                
                $('input[name*="web"]').each(function(i){
                    var int_name = $(this).attr('name');
                    var int_time = int_name.split('-');
                    interval = int_time[2] < 10 ? '0'+int_time[2] : int_time[2];
                    var sel_datetime = sel_date+interval;                                 
                    if(svr_datetime > sel_datetime){
                        $('input[name*="'+int_time[2]+'"]').prop('disabled','disabled');                
                        $('input[name="go-'+int_time[2]+'"]').prop('disabled','disabled');
                        $('select[name=action]').prop('disabled','disabled')        
                        $('#submit_offer').prop('disabled','disabled')        
                    }else{
                        $('input[name*="'+int_time[2]+'"]').prop('disabled',false);
                        $('input[name*="go-'+int_time[2]+'"]').prop('disabled',false);
		...
		// // REALTIME DETERMINE DISABLED FIELDS
		socket.on("app.common.time:App\\Events\\ServerTime", function(message){
			var svr_time = message.data.split(':');
            var svr_datetime = moment().hours(svr_time[0]).format('YYYY-MM-DD HH');
            var svr_datetime = moment(svr_datetime).add(3, 'hours').format('YYYYMMDDHH');
            var sel_date = $('#delivery_date').val();
            $('input[name*="web"]').each(function(i){
                var int_name = $(this).attr('name');
                ...
                interval = int_time[2] < 10 ? '0'+int_time[2] : int_time[2];
                var sel_datetime = sel_date+interval;
                // console.log(svr_datetime+'----------'+sel_datetime);
                // console.log(sel_datetime+'----------'+svr_datetime);
                if(svr_datetime > sel_datetime){
                    $('input[name*="'+int_time[2]+'"]').prop('disabled','disabled');                
                    $('input[name="go-'+int_time[2]+'"]').prop('disabled','disabled');
                    $('select[name=action]').prop('disabled','disabled')        
