...
<style>
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-10">
                <legend>Scheduled Offers</legend>
                @php
                $form_data = array();
                ...
                for($i=1;$i<=24;$i++){
                    ...
                    for($n=1;$n<=12;$n++){
                        $min = $min+5;
                        if(date('H:i',mktime($i-1,$min)) == '00:00'){
                            $intra = "24:00";
                        }else{
                            $intra = date('H:i',mktime($i-1,$min));
                        }

                        $form_data['intervals'][$i][$intra] = array(
                                'price_quantity' => '',
                                'ramp_rate' => '',
                                'remarks' => ''
                        );
                    ...
                    {{ Form::open(['class'=>'form-horizontal','id' => 'form_retrieve']) }}
                        <div class="row">
                            <div class="form-group col-md-5"> 
                                ...
                                <div class="col-lg-6">
                                    {{ Form::select('delivery_date',$arr_date,($form_data['delivery_date'] ? $form_data['delivery_date']->format('Ymd') : date('Ymd')),['class'=>'form-control input-sm']) }}
                                </div>
                            </div>
                        </div>
                        <div class="row">
                ...
                {{-- <div class="well col-md-12">
                    <h5>Upload Day Ahead Offer Template &nbsp;&nbsp;
                    ...
                    <div id="collapseOne" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
                          <div class="panel-body">
                            {{ Form::open(array('route'=>'day_ahead_offer.upload','files'=>true,'method'=>'POST'),['class'=>'form-horizontal']) }}
                            {{ Form::hidden('upload_action','')}}
                            {{ Form::hidden('template_type','day_ahead')}}
                    ...
                    <div id="collapsePopulate" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
                          <div class="panel-body">
                            ...
                            <div class="col-lg-12">
                                <div class="row">                               
                                    <div class="form-group col-lg-10">
                                        ...
                                        <div class="col-lg-10 input-group">
                                            {{ Form::text('web-price_qty', '(2000.00,120.0),(2000.00,125.0),(3000.00,300.0);', ['class'=>'form-control input-sm', 'placeholder'=>'Price/Quantity']) }}  
                                ...
                                <div class="row">                               
                                    <div class="form-group col-lg-10">
                                        {{ Form::label('rr', 'Ramp Rate:', ['class'=>'col-lg-2 control-label']) }}
                                        <div class="col-lg-10 input-group">
                                            {{ Form::text('web-ramp_rate', '(300.0,3.0,3.0);', ['class'=>'form-control input-sm', 'placeholder'=>'Ramp Rate']) }}
                                            <label class="input-group-addon">{{ Form::checkbox('web-ramp_rate_cb','',true)}}</label>        
                                        </div>                                                                              
                                    </div>
                                </div>
                                <div class="row">                               
                                    <div class="form-group col-lg-10">
                                        ...
                                        <div class="col-lg-10 input-group">
                                            {{ Form::text('web-remarks', 'NET GENERATION', ['class'=>'form-control input-sm', 'placeholder'=>'Remarks']) }}
                        ...
                        <div class="tab-pane active" id="web_content">
                            <table class="table table-hover table-striped table-condensed">
                                ...
                                <tr >
                                    ...
                                    <th width="5%">Interval {{-- <br /><label><input id="check_all" type="checkbox" /> All</label> --}}</th>
                                    <th width="54%">Price / Quantity</th>
                                    <th width="15%">Ramp Rate</th>
                                ...
                                <tbody>                                                                                                         
                                    @for($i=1; $i<=24; $i++)   
                                        ...
                                        @endphp                                                             
                                        <tr id="tr{{ $i }}">
                                            <td rowspan="13" class="hour-label">
                                                <input name="go-{{ $i }}" id="go-{{ $i }}" type="checkbox" value="1" />
                                                 {{ $i }}
                                            </td>
                                            @for($n=1; $n<=12; $n++)
                                            <tr>
                                                <td>
                                                    ...
                                                    @endphp
                                                    <input name="int-{{ $i }}-{{ $intra }}" id="int-{{ $i }}-{{ $intra }}" type="checkbox" value="1" />
                                                    {{ $intra }}
                                                </td>
                                                <td>
                                                    <input name="web-price_qty-{{ $i }}-{{ $intra }}" type="text" class="form-control price_qty input-sm" value="{{ $form_data['intervals'][$i][$intra]['price_quantity']  ? $form_data['intervals'][$i][$intra]['price_quantity'] : ''}}"/>
                                                </td>
                                                <td>
                                                    <input name="web-ramp_rate-{{ $i }}-{{ $intra }}" type="text" class="form-control ramp_rate input-sm" value="{{ $form_data['intervals'][$i][$intra]['ramp_rate']  ? $form_data['intervals'][$i][$intra]['ramp_rate'] : ''}}"/>
                                                </td>
                                                <td>
                                                    <input name="web-remarks-{{ $i }}-{{ $intra }}" type="text" class="form-control remarks input-sm" value="{{ $form_data['intervals'][$i][$intra]['remarks']  ? $form_data['intervals'][$i][$intra]['remarks'] : ''}}"/>
                                                </td>
                                            </tr>
                                            @endfor
                                        </tr>
   ...
   $(document).ready(function(){
        $.extend({
            ...
            determineDisabled : function(){
                /*$.ajax({
                   url : "{{ route('common.get_time') }}",
                   error : function(error){
                    console.log(error);               
                   }
                });*/
                var svr_time_full = "{{ $cur_time }}";
                var svr_time = svr_time_full.split(':');
                var svr_datetime = moment().hours(svr_time[0]).minutes(svr_time[1]).format('YYYY-MM-DD H:mm');
                var svr_datetime = moment(svr_datetime).subtract(10, 'minutes').format('YYYYMMDDHmm');
                var sel_date = $('#delivery_date').val();
                   
                $('input[name*="int"]').each(function(i){
                    var int_name = $(this).attr('name');
                    var int_time = int_name.split('-');
                    var int_hrmin = int_time[2].split(":")
                    var sel_datetime = sel_date+int_hrmin[0]+int_hrmin[1];
                        if(svr_datetime > sel_datetime){
                            $('input[name*="'+int_time[2]+'"]').prop('disabled','disabled');                
                            $('input[name="go-'+int_time[1]+'"]').prop('disabled','disabled');
                            $('select[name=action]').prop('disabled','disabled')        
                            $('#submit_offer').prop('disabled','disabled')        
                        }else{
                            $('input[name*="'+int_time[2]+'"]').prop('disabled',false);
                            $('input[name*="go-'+int_time[1]+'"]').prop('disabled',false);
                            $('select[name=action]').prop('disabled',false)
        ...
        socket.on("app.common.time:App\\Events\\ServerTime", function(message){
           var svr_time = message.data.split(':');
           var svr_datetime = moment().hours(svr_time[0]).minutes(svr_time[1]).format('YYYY-MM-DD H:mm');
           var svr_datetime = moment(svr_datetime).subtract(10, 'minutes').format('YYYYMMDDHmm');
           var sel_date = $('#delivery_date').val();
        ...
        $('input[name*=go]').bind('change',function(){
            var cb_name = $(this).attr("name");            
            var cb_hour = cb_name.split('-');
            var hr = cb_hour[1];           
            var chk = $("input[name=go-"+hr).is(':checked');            
            if(chk){
                $('#form_submit input[name*=int-'+hr+']').prop('checked', 'checked');
            }else{
                $('#form_submit input[name*=int-'+hr+']').prop('checked',false);
            }
